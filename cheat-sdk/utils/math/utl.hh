...
public:
    ...
    [[nodiscard]] static I get_index( const iterator_t& it ) { return it.index; }
    [[nodiscard]] static bool is_idx_after( I i, const iterator_t& it ) { return i > it.index; }
    ...
    static void validate_grow_size( ) {}
...
c_utl_memory< T, I >::c_utl_memory( const int grow_size, const int init_allocation_count ) :
    m_memory( nullptr ),
    m_allocation_count_( init_allocation_count ),
...
template < class T, class I > c_utl_memory< T, I >::c_utl_memory( T* memory, int num_elements ) :
    m_memory( memory ),
...
template < class T, class I > c_utl_memory< T, I >::c_utl_memory( const T* memory, int num_elements ) :
    m_memory( ( T* ) memory ),
    m_allocation_count_( num_elements ) {
    // Special marker indicating externally supplied modifyable memory
...
template < class T, class I > c_utl_memory< T, I >::~c_utl_memory( ) {
    purge( );
}

template < class T, class I > void c_utl_memory< T, I >::init( int grow_size /*= 0*/, int init_size /*= 0*/ ) {
    purge( );
...
// Fast swap
//-----------------------------------------------------------------------------
template < class T, class I > void c_utl_memory< T, I >::swap( c_utl_memory< T, I >& mem ) noexcept {
...
//-----------------------------------------------------------------------------
template < class T, class I > void c_utl_memory< T, I >::convert_to_growable_memory( const int grow_size ) {
...
//-----------------------------------------------------------------------------
template < class T, class I > void c_utl_memory< T, I >::set_external_buffer( T* memory, const int num_elements ) {
...
template < class T, class I > void c_utl_memory< T, I >::set_external_buffer( const T* memory, int num_elements ) {
    // Blow away any existing allocated memory
    purge( );
...
template < class T, class I > void c_utl_memory< T, I >::assume_memory( T* memory, int size ) {
    // Blow away any existing allocated memory
    purge( );

    // Simply take the pointer but don't mark us as external
    m_memory = memory;
    m_allocation_count_ = size;
}

template < class T, class I > void* c_utl_memory< T, I >::detach_memory( ) {
    if ( is_externally_allocated( ) )
        return nullptr;
...
template < class T, class I > T* c_utl_memory< T, I >::detach( ) {
...
template < class T, class I > T& c_utl_memory< T, I >::element( I i ) {
    assert( !is_read_only( ) );
    assert( is_idx_valid( i ) );
    return m_memory[ i ];
}

template < class T, class I > const T& c_utl_memory< T, I >::element( I i ) const {
    assert( is_idx_valid( i ) );
    return m_memory[ i ];
}
...
//-----------------------------------------------------------------------------
template < class T, class I > bool c_utl_memory< T, I >::is_externally_allocated( ) const {
...
//-----------------------------------------------------------------------------
template < class T, class I > bool c_utl_memory< T, I >::is_read_only( ) const {
    return ( m_grow_size == external_const_buffer_marker );
}

template < class T, class I > void c_utl_memory< T, I >::set_grow_size( const int size ) {
...
//-----------------------------------------------------------------------------
template < class T, class I > T* c_utl_memory< T, I >::base( ) {
    assert( !is_read_only( ) );
    return m_memory;
}

template < class T, class I > const T* c_utl_memory< T, I >::base( ) const {
    return m_memory;
}
...
// Size
//-----------------------------------------------------------------------------
template < class T, class I > int c_utl_memory< T, I >::num_allocated( ) const {
    return m_allocation_count_;
}

template < class T, class I > int c_utl_memory< T, I >::count( ) const {
    return m_allocation_count_;
}
...
//-----------------------------------------------------------------------------
template < class T, class I > bool c_utl_memory< T, I >::is_idx_valid( I i ) const {
...
// Grows the memory
//-----------------------------------------------------------------------------
inline int utl_memory_calc_new_allocation_count( int allocation_count, const int grow_size, const int new_size,
...
template < class T, class I > void c_utl_memory< T, I >::grow( int num ) {
    assert( num > 0 );
    ...
    int new_allocation_count =
        utl_memory_calc_new_allocation_count( m_allocation_count_, m_grow_size, allocation_requested, sizeof( T ) );

    // if m_nAllocationRequested wraps index type I, recalculate
    if ( ( int ) ( I ) new_allocation_count < allocation_requested ) {
        if ( ( int ) ( I ) new_allocation_count == 0
             && ( int ) ( I ) ( new_allocation_count - 1 ) >= allocation_requested ) {
...
//-----------------------------------------------------------------------------
template < class T, class I > void c_utl_memory< T, I >::ensure_capacity( int num ) {
...
// Memory deallocation
//-----------------------------------------------------------------------------
template < class T, class I > void c_utl_memory< T, I >::purge( ) {
    if ( !is_externally_allocated( ) ) {
        if ( m_memory ) {
            free( ( void* ) m_memory );
            m_memory = 0;
...
template < class T, class I > void c_utl_memory< T, I >::purge( int num_elements ) {
...
//-----------------------------------------------------------------------------
template < class T, int nAlignment > class c_utl_memory_aligned : public c_utl_memory< T > {
public:
    // constructor, destructor
...
// Aligns a pointer
//-----------------------------------------------------------------------------
template < class T, int nAlignment > void* c_utl_memory_aligned< T, nAlignment >::align( const void* pAddr ) {
...
c_utl_memory_aligned< T, nAlignment >::c_utl_memory_aligned( T* memory, int num_elements ) {
    ...
    c_utl_memory< T >::m_nAllocationcount =
        ( ( int ) ( memory + num_elements ) - ( int ) c_utl_memory< T >::m_memory ) / sizeof( T );
}
...
template < class T, int nAlignment > c_utl_memory_aligned< T, nAlignment >::~c_utl_memory_aligned( ) {
    purge( );
}
...
// Grows the memory
//-----------------------------------------------------------------------------
template < class T, int nAlignment > void c_utl_memory_aligned< T, nAlignment >::grow( int num ) {
    assert( num > 0 );
    ...
    c_utl_memory< T >::m_nAllocationcount = utl_memory_calc_new_allocation_count(
        c_utl_memory< T >::m_nAllocationcount, c_utl_memory< T >::m_grow_size, allocation_requested, sizeof( T ) );

    if ( c_utl_memory< T >::m_memory ) {
        c_utl_memory< T >::m_memory = ( T* ) MemAlloc_ReallocAligned(
            c_utl_memory< T >::m_memory, c_utl_memory< T >::m_nAllocationcount * sizeof( T ), nAlignment );
        assert( CUtlMemory< T >::m_memory );
    } else {
        c_utl_memory< T >::m_memory =
            ( T* ) MemAlloc_AllocAligned( c_utl_memory< T >::m_nAllocationcount * sizeof( T ), nAlignment );
        assert( CUtlMemory< T >::m_memory );
    }
}
...
// Makes sure we've got at least this much memory
//-----------------------------------------------------------------------------
template < class T, int nAlignment > void c_utl_memory_aligned< T, nAlignment >::ensure_capacity( int num ) {
...
// Memory deallocation
//-----------------------------------------------------------------------------
template < class T, int nAlignment > void c_utl_memory_aligned< T, nAlignment >::purge( ) {
    if ( !this->IsExternallyAllocated( ) ) {
        if ( c_utl_memory< T >::m_memory ) {
...
public:
    ...
    void set_count_non_destructively( int count ); // sets count by adding or removing elements to tail TODO: This
                                                   // should probably be the default behavior for Setcount
...
public:
    ...
    void reset_dbg_info( ) { m_elements = base( ); }
};
...
c_utl_vector< T, A >::c_utl_vector( int grow_size, int init_size ) : m_memory( grow_size, init_size ), m_size( 0 ) {
    reset_dbg_info( );
}

template < typename T, class A >
c_utl_vector< T, A >::c_utl_vector( T* memory, int allocation_count, const int num_elements ) :
    m_memory( memory, allocation_count ),
    m_size( num_elements ) {
    reset_dbg_info( );
}

template < typename T, class A > c_utl_vector< T, A >::~c_utl_vector( ) {
    purge( );
}
...
c_utl_vector< T, A >& c_utl_vector< T, A >::operator=( const c_utl_vector< T, A >& other ) {
    ...
    set_size( ncount );
    for ( int i = 0; i < ncount; i++ ) { ( *this )[ i ] = other[ i ]; }
    return *this;
}
...
template < typename T, class A > T& c_utl_vector< T, A >::element( int i ) {
    assert( i < m_size );
    return m_memory[ i ];
}

template < typename T, class A > const T& c_utl_vector< T, A >::element( int i ) const {
    assert( i < m_size );
    return m_memory[ i ];
}

template < typename T, class A > T& c_utl_vector< T, A >::head( ) {
    assert( m_size > 0 );
    return m_memory[ 0 ];
}

template < typename T, class A > const T& c_utl_vector< T, A >::head( ) const {
    assert( m_size > 0 );
    return m_memory[ 0 ];
}

template < typename T, class A > T& c_utl_vector< T, A >::tail( ) {
    assert( m_size > 0 );
    return m_memory[ m_size - 1 ];
}

template < typename T, class A > const T& c_utl_vector< T, A >::tail( ) const {
    assert( m_size > 0 );
    return m_memory[ m_size - 1 ];
}
...
// count
//-----------------------------------------------------------------------------
template < typename T, class A > inline int c_utl_vector< T, A >::count( ) const {
    return m_size;
}
...
// Is element index valid?
//-----------------------------------------------------------------------------
template < typename T, class A > inline bool c_utl_vector< T, A >::is_valid_index( int i ) const {
    return ( i >= 0 ) && ( i < m_size );
}
...
//-----------------------------------------------------------------------------
template < typename T, class A > int c_utl_vector< T, A >::invalid_index( ) {
    return -1;
}
...
// Grows the vector
//-----------------------------------------------------------------------------
template < typename T, class A > void c_utl_vector< T, A >::grow_vector( int num ) {
...
// Sorts the vector
//-----------------------------------------------------------------------------
template < typename T, class A > void c_utl_vector< T, A >::sort( int( __cdecl* pfn_compare )( const T*, const T* ) ) {
...
// Makes sure we have enough memory allocated to store a requested # of elements
//-----------------------------------------------------------------------------
template < typename T, class A > void c_utl_vector< T, A >::ensure_capacity( int num ) {
...
// Makes sure we have at least this many elements
//-----------------------------------------------------------------------------
template < typename T, class A > void c_utl_vector< T, A >::ensure_count( const int num ) {
    if ( count( ) < num ) {
...
// Shifts elements
//-----------------------------------------------------------------------------
template < typename T, class A > void c_utl_vector< T, A >::shift_elements_right( const int elem, const int num ) {
...
template < typename T, class A > void c_utl_vector< T, A >::shift_elements_left( const int elem, int num ) {
    assert( is_valid_index( elem ) || ( m_size == 0 ) || ( num == 0 ) );
    const int num_to_move = m_size - elem - num;
    if ( ( num_to_move > 0 ) && ( num > 0 ) ) {
...
// Adds an element, uses default constructor
//-----------------------------------------------------------------------------
template < typename T, class A > int c_utl_vector< T, A >::add_to_head( ) {
    return insert_before( 0 );
}

template < typename T, class A > int c_utl_vector< T, A >::add_to_tail( ) {
    return insert_before( m_size );
}

template < typename T, class A > int c_utl_vector< T, A >::insert_after( const int elem ) {
    return insert_before( elem + 1 );
}

template < typename T, class A > int c_utl_vector< T, A >::insert_before( const int elem ) {
...
// Adds an element, uses copy constructor
//-----------------------------------------------------------------------------
template < typename T, class A > int c_utl_vector< T, A >::add_to_head( const T& src ) {
    // Can't insert something that's in the list... reallocation may hose us
    assert( ( base( ) == NULL ) || ( &src < base( ) ) || ( &src >= ( base( ) + count( ) ) ) );
    return insert_before( 0, src );
}

template < typename T, class A > int c_utl_vector< T, A >::add_to_tail( const T& src ) {
    // Can't insert something that's in the list... reallocation may hose us
    assert( ( base( ) == NULL ) || ( &src < base( ) ) || ( &src >= ( base( ) + count( ) ) ) );
    return insert_before( m_size, src );
}

template < typename T, class A > int c_utl_vector< T, A >::insert_after( const int elem, const T& src ) {
    // Can't insert something that's in the list... reallocation may hose us
    assert( ( base( ) == NULL ) || ( &src < base( ) ) || ( &src >= ( base( ) + count( ) ) ) );
...
// Adds multiple elements, uses default constructor
//-----------------------------------------------------------------------------
template < typename T, class A > int c_utl_vector< T, A >::add_multiple_to_head( const int num ) {
    return insert_multiple_before( 0, num );
}

template < typename T, class A > int c_utl_vector< T, A >::add_multiple_to_tail( const int num ) {
    return insert_multiple_before( m_size, num );
}

template < typename T, class A > int c_utl_vector< T, A >::add_multiple_to_tail( const int num, const T* pToCopy ) {
    // Can't insert something that's in the list... reallocation may hose us
...
template < typename T, class A > int c_utl_vector< T, A >::insert_multiple_after( const int elem, const int num ) {
    return insert_multiple_before( elem + 1, num );
}

template < typename T, class A > void c_utl_vector< T, A >::set_count( int count ) {
    remove_all( );
    add_multiple_to_tail( count );
}

template < typename T, class A > void c_utl_vector< T, A >::set_size( int size ) {
    set_count( size );
}

template < typename T, class A > void c_utl_vector< T, A >::set_count_non_destructively( int count ) {
...
template < typename T, class A > void c_utl_vector< T, A >::copy_array( const T* pArray, int size ) {
    // Can't insert something that's in the list... reallocation may hose us
    assert( ( base( ) == NULL ) || !pArray || ( base( ) >= ( pArray + size ) )
            || ( pArray >= ( base( ) + count( ) ) ) );

    set_size( size );
    for ( int i = 0; i < size; i++ ) { ( *this )[ i ] = pArray[ i ]; }
}

template < typename T, class A > void c_utl_vector< T, A >::swap( c_utl_vector< T, A >& vec ) noexcept {
...
template < typename T, class A > int c_utl_vector< T, A >::add_vector_to_tail( c_utl_vector const& src ) {
    assert( &src != this );
    ...
    // Copy the elements.
    m_size += src_count;
    for ( int i = 0; i < src_count; i++ ) { CopyConstruct( &element( base + i ), src[ i ] ); }
    return base;
}
template < class T > inline T* сonstruct( T* pMemory ) {
    return ::new ( pMemory ) T;
}
template < typename T, class A > int c_utl_vector< T, A >::insert_multiple_before( const int elem, int num ) {
    if ( num == 0 )
        return elem;
    ...
    for ( int i = 0; i < num; ++i ) { сonstruct( &element( elem + i ) ); }
...
int c_utl_vector< T, A >::insert_multiple_before( int elem, int num, const T* pToInsert ) {
    ...
    if ( !pToInsert ) {
        ...
        for ( int i = 0; i < num; i++ ) { CopyConstruct( &element( elem + i ), pToInsert[ i ] ); }
    }
...
// Finds an element (element needs operator== defined)
//-----------------------------------------------------------------------------
template < typename T, class A > int c_utl_vector< T, A >::get_offset( const T& src ) const {
...
template < typename T, class A > void c_utl_vector< T, A >::fill_with_value( const T& src ) {
    for ( int i = 0; i < count( ); i++ ) { element( i ) = src; }
}

template < typename T, class A > bool c_utl_vector< T, A >::has_element( const T& src ) const {
...
// element removal
//-----------------------------------------------------------------------------
template < typename T, class A > void c_utl_vector< T, A >::fast_remove( int elem ) {
...
template < typename T, class A > void c_utl_vector< T, A >::remove( int elem ) {
    Destruct( &element( elem ) );
    shift_elements_left( elem );
    --m_size;
}

template < typename T, class A > bool c_utl_vector< T, A >::find_and_remove( const T& src ) {
...
template < typename T, class A > bool c_utl_vector< T, A >::find_and_fast_remove( const T& src ) {
    const int elem = get_offset( src );
    if ( elem != -1 ) {
        fast_remove( elem );
        return true;
...
template < typename T, class A > void c_utl_vector< T, A >::remove_multiple( const int elem, const int num ) {
    assert( elem >= 0 );
...
template < typename T, class A > void c_utl_vector< T, A >::remove_multiple_from_head( const int num ) {
    assert( num <= count( ) );
...
template < typename T, class A > void c_utl_vector< T, A >::remove_multiple_from_tail( const int num ) {
    assert( num <= count( ) );
...
// gcc doesn't allow storage specifiers on explicit template instatiation, but visual studio needs them to avoid link
// errors.
...
#define FMTFUNCTION( fmtargnumber, firstvarargnumber ) \
    __attribute__( ( format( printf, fmtargnumber, firstvarargnumber ) ) )
...
//
// Note: NJS: This is not enabled for regular PC, due to not knowing the implications of exporting a class with no no
// vtable.
...
#pragma warning( \
    disable : 4239 ) // nonstandard extension used : 'argument' ( conversion from class Vector to class Vector& )
...
#pragma warning( disable : 4530 ) // warning C4530: C++ exception handler used, but unwind semantics are not enabled.
                                  // Specify /EHsc (disabled due to std headers having exception syntax)
#endif
...
template < typename T > T WordSwapC( T w ) {
...
template < typename T > T DWordSwapC( T dw ) {
...
template < typename T > inline T WordSwap360Intr( T w ) {
    T output;
    __storeshortbytereverse( w, 0, &output );
    return output;
}

template < typename T > inline T DWordSwap360Intr( T dw ) {
    T output;
...
template < typename T > inline T WordSwapAsm( T w ) {
    __asm
    {
		mov ax, w
		xchg al, ah
...
template < typename T > inline T DWordSwapAsm( T dw ) {
    __asm
    {
		mov eax, dw
		bswap eax
...
template < class T, typename ARG1 > T* construct( T* memory, ARG1 a1 ) {
    return ::new ( memory ) T( a1 );
}

template < class T, typename ARG1, typename ARG2 > T* construct( T* memory, ARG1 a1, ARG2 a2 ) {
    return ::new ( memory ) T( a1, a2 );
}

template < class T, typename ARG1, typename ARG2, typename ARG3 > T* construct( T* memory, ARG1 a1, ARG2 a2, ARG3 a3 ) {
...
#define TEMPLATE_FUNCTION_TABLE( RETURN_TYPE, NAME, ARGS, count )                        \
    ...
    typedef RETURN_TYPE( FASTCALL* __Type_##NAME ) ARGS;                                 \
                                                                                         \
    template < const int nArgument > struct __Function_##NAME {                          \
        static RETURN_TYPE FASTCALL Run ARGS;                                            \
    };                                                                                   \
                                                                                         \
    template < const int i > struct __MetaLooper_##NAME : __MetaLooper_##NAME< i - 1 > { \
        __Type_##NAME func;                                                              \
        inline __MetaLooper_##NAME( ) { func = __Function_##NAME< i >::Run; }            \
    };                                                                                   \
                                                                                         \
    template <> struct __MetaLooper_##NAME< 0 > {                                        \
        __Type_##NAME func;                                                              \
    ...
    template < const int nArgument > RETURN_TYPE FASTCALL __Function_##NAME< nArgument >::Run ARGS
...
template < typename T, class A > void c_utl_vector< T, A >::remove_all( ) {
...
template < typename T, class A > void c_utl_vector< T, A >::purge( ) {
    remove_all( );
    m_memory.purge( );
    reset_dbg_info( );
}

template < typename T, class A > void c_utl_vector< T, A >::purge_and_delete_elements( ) {
    for ( int i = 0; i < m_size; i++ ) { delete element( i ); }
    purge( );
}

template < typename T, class A > void c_utl_vector< T, A >::compact( ) {
    m_memory.purge( m_size );
}

template < typename T, class A > int c_utl_vector< T, A >::num_allocated( ) const {
...
template < typename T, class A > void CUtlVector< T, A >::Validate( CValidator& validator, char* pchName ) {
...
public:
    ~c_utl_vector_auto_purge( void ) { this->PurgeAndDeleteelements( ); }
};
...
public:
    ...
    static int __cdecl sort_func( char* const* sz1, char* const* sz2 ) { return strcmp( *sz1, *sz2 ); }
};
