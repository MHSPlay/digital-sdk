...
#define xorstr( str )                                                                                        \
    ::jm::make_xorstr( []( ) { return str; }, std::make_index_sequence< sizeof( str ) / sizeof( *str ) >{ }, \
...
namespace jm {
    ...
    namespace detail {
        ...
        template <> struct unsigned_< 1 > {
            using type = std::uint8_t;
        };
        template <> struct unsigned_< 2 > {
            using type = std::uint16_t;
        };
        template <> struct unsigned_< 4 > {
            using type = std::uint32_t;
        };

        template < auto C, auto... > struct pack_value_type {
            using type = decltype( C );
        };

        template < std::size_t Size > XORSTR_FORCEINLINE constexpr std::size_t _buffer_size( ) {
        ...
        template < std::uint32_t Seed > XORSTR_FORCEINLINE constexpr std::uint32_t key4( ) noexcept {
            std::uint32_t value = Seed;
            for ( char c : __TIME__ ) value = static_cast< std::uint32_t >( ( value ^ c ) * 16777124619ull );
            return value;
        }
        ...
        template < class T >
        XORSTR_FORCEINLINE constexpr std::uint64_t load_xored_str8( std::uint64_t key, std::size_t idx ) noexcept {
            ...
            for ( std::size_t i = 0; i < idx_offset && i + idx * idx_offset < T::size; ++i )
                value ^= ( std::uint64_t{ static_cast< cast_type >( T::str[ i + idx * idx_offset ] ) }
    ...
    public:
        ...
        XORSTR_FORCEINLINE xor_string( ) noexcept :
            _storage{ detail::load_from_reg(
                detail::uint64_v< detail::load_xored_str8< T >( Keys::key, Keys::idx ) >::value )... } {}

        XORSTR_FORCEINLINE constexpr size_type size( ) const noexcept { return T::size - 1; }

        XORSTR_FORCEINLINE void crypt( ) noexcept {
#if defined( __clang__ )
            alignas( T::buffer_align ) std::uint64_t arr[ sizeof...( Keys ) ]{ detail::load_from_reg( Keys::key )... };
            std::uint64_t* keys = ( std::uint64_t* ) detail::load_from_reg( ( std::uint64_t ) arr );
#else
            alignas( T::buffer_align ) std::uint64_t keys[ sizeof...( Keys ) ]{ detail::load_from_reg( Keys::key )... };
#endif
    ...
    XORSTR_FORCEINLINE constexpr auto make_xorstr( Tstr str_lambda, std::index_sequence< StringIndices... >,
