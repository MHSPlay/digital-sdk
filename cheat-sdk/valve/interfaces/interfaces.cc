...
template < typename T > T c_interfaces::get( const c_module_info& the_module, const std::string_view interface_name ) {
    ...
    const auto& create_interface_fn = c_address( create_interface ).add( 0x4 ).relative( );
    if ( !create_interface_fn ) 
        return T( );

    const auto interface_reg = create_interface_fn.add( 0x6 ).get< c_interface_reg* >( 2 );
...
void c_interfaces::init( ) {
    ...
    m_model_render = get< i_model_render* const >(
        static_cast< c_module_info >( g_modules->m_engine_dll ), _( "VEngineModel" ) );

    m_globals = g_mem->get_vfunc( m_client, 11 ).add( 0xA ).get< c_global_vars* >( 2 );
    m_client_state = g_mem->get_vfunc( m_engine, 12 ).add( 0x10 ).get< c_client_state* >( 2 );
    m_input = g_mem->get_vfunc( m_client, 16 ).at< i_input* >( 0x1 );
    m_client_mode = g_mem->get_vfunc( m_client, 10 ).add( 0x5 ).get< i_client_mode* >( 2 );

    m_move_data = g_modules->m_client_dll.get_address( _( "g_pMoveData" ) ).add( 0x2 ).get< c_move_data* >( 2 );
    m_move_helper = g_modules->m_client_dll.get_address( _( "g_pMoveHelper" ) ).add( 0x2 ).get< i_move_helper* >( 2 );
    m_device = g_modules->m_shaderapidx9_dll.get_address( _( "g_pDirect3DDevice9" ) ).add( 0x1 ).get< IDirect3DDevice9* >( 2 );
