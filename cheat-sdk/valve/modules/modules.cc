...
bool c_modules::get( ) {
    ...
    return m_client_dll.get_module( ) && m_gameoverlayrenderer_dll.get_module( ) && m_engine_dll.get_module( ) &&
        m_steam_api_dll.get_module( ) && m_vstd_dll.get_module( ) && m_vguimatsurface_dll.get_module( ) &&
        m_data_cahe_dll.get_module( ) && m_materialsystem_dll.get_module( ) && m_vphysics_dll.get_module( ) &&
        m_shaderapidx9_dll.get_module( );
}
...
bool c_modules::get_addresses( ) {
    ...
    m_client_dll.add_address( _( "C_BaseEntity::PhysicsRunThink" ), _( "E8 ? ? ? ? 84 C0 0F 84 ? ? ? ? F3 0F 10 86 ? ? ? ?" ), true );
...
bool c_loaded_module::hook_function( const std::string_view name, void* custom_function, void* target ) {
    auto function = !target ? m_addresses[ fnv1a::hash( name ) ].as< void* >( ) : target;
    if ( !function )
        return false;

    static auto hook = g_modules->m_gameoverlayrenderer_dll.get_address( _( "GameOverlayRenderer::HookFunc" ) ).as<
        bool( __cdecl *)( void*, void*, void*, int ) >( );
    if ( !hook( function, custom_function, &m_originals[ fnv1a::hash( name ) ], 0 ) )
        return false;
...
void c_loaded_module::unload_functions( ) {
    ...
    // @todo: add game overlay renderer unhooker
...
bool c_loaded_module::valid_addresses( ) {
    return std::ranges::all_of(
        m_addresses | std::views::values, []( const c_address& address ) {
