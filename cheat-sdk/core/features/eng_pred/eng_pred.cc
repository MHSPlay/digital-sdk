...
void c_eng_pred::begin( ) {
    if ( !c_player::get_local( ) || !c_player::get_local( )->is_alive( ) || !g_ctx->m_cmd )
        return;

    m_backup_data.store( );

    c_player::get_local( )->current_cmd( ) = g_ctx->m_cmd;
    c_player::get_local( )->last_cmd( ) = g_ctx->m_cmd;

    c_player::get_local( )->set_pred_player( c_player::get_local( ) );
    c_player::get_local( )->set_pred_random_seed( g_ctx->m_cmd );
    ...
    g_ctx->m_cmd->m_buttons |= c_player::get_local( )->button_forced( );
    g_ctx->m_cmd->m_buttons &= ~c_player::get_local( )->button_disabled( );
    ...
    if ( g_ctx->m_cmd->m_impulse )
        c_player::get_local( )->impulse( ) = g_ctx->m_cmd->m_impulse;

    c_player::get_local( )->update_button_state( g_ctx->m_cmd->m_buttons );
    ...
    g_interfaces->m_prediction->set_local_view_angles( g_ctx->m_cmd->m_view_angles );
    ...
    g_interfaces->m_prediction->setup_move( c_player::get_local( ), g_ctx->m_cmd, g_interfaces->m_move_helper,
                                            g_interfaces->m_move_data );
    g_interfaces->m_game_movement->process_movement( c_player::get_local( ), g_interfaces->m_move_data );
    g_interfaces->m_prediction->finish_move( c_player::get_local( ), g_ctx->m_cmd, g_interfaces->m_move_data );
